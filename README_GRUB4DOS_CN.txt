...
选项：
	...
	--usb=I   		将 usb2.0 驱动程序安装到指定的硬盘驱动器或硬盘映像（设备或文件）。

	--grldr-usb=I		在 grldr 加载 usb2.0 的选项。
...
更新：增加 --usb=I 参数，可以在 FAT16/32/exfat 安装 usb2.0 驱动。其中 I 表示：
	01：当启动设备被识别为软盘时，加载；
	02：无条件加载。
...
***                           关于 usb2.0 驱动程序                         ***
******************************************************************************
当 bios 分配驱动器号为 00，又不支持扩展读时，chs 参数识别错误会造成 u 盘启动失败。
作为一种补救措施，加载 usb 驱动后，可以支持扩展读，驱动器号由 00 改为 80，解决 chs 识别问题。

usb 驱动代码位于 grldr.pbr 的第11至21扇区。

usb2.0 驱动支持：PCI 分类代码的 0c/03/20，即 ehci（增强总线控制接口）设备。
usb2.0 驱动支持：usb（通用串行总线）的接口类型08（大容量存储类）、子类06、协议50，
即通常的u盘或移动硬盘。

支持 usb-hdd, usb-cdrom 模式。

usb 驱动代码放在 PBR 扇区的后面。
对于 FAT16/FAT32/exFAT，分别位于分区 LBA(1)/LBA(2)/LBA(24)。

PBR 引导程序首先加载 usb 驱动到 xxx0:0000（需要 0x3400 字节），然后使用远调用，驻留在常规内存顶部，
通过拦截 int13 来读写 usb 设备。 

在 PBR 的偏移 0x1fb 处有一开关。
  01表示：当启动设备被识别为软盘时，加载；
  02表示：无条件加载；
  00表示：不加载。

在加载 usb 驱动之初，还有一次5秒提示：按 s 键降速加载；按空格键不加载；默认加载。
等待秒数可由 usb 驱动程序的偏移 17 处设置。
速度设定可由 usb 驱动程序的偏移 18 处设置。（0=全速，1=全速/2，2=全速/4，3=全速/8）

	--usb-time=T 	T是等待秒数
	--usb-speed=S	S是速度

注意：现在只有文件系统（FAT16/FAT32/exFAT）被支持。
注意：u盘预先需要格式化，取得 MBR 及 BPB 参数。
      
贴士：1. 有些 u 盘被 Windows 或 DOS 下的 usbaspi.sys 识别为 usb1.0 设备，但是
         降速加载后会重新识别为 usb2.0 设备。
      2. 有些 u 盘插入计算机前端插口不被识别，但是降速加载后会被识别。
      3. 加载失败时，按 s 键试一试。

更新：grldr 内含 usb2.0 驱动，加载条件同 PBR。由 --grldr-usb=I 设置。
      F2 键无条件加载(通过偏移 0x1f9 可重新设置）。
	--usb-key=K	K是扫描码
例：bootlace --grldr-usb=2 grldr
