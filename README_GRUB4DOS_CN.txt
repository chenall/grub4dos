...
卷的根目录，并按照grldr.dbr的第3至4扇区建立它的引导扇区。那么，这个EXT2/3/4卷就是GRUB可启动的。
...
02h	1	LBA指示码。CHS模式时，有效值是0x02；LBA模式时有效值为0x42。
...
03h	10	OEM 名称字符串（对该磁盘进行格式化的操作系统的名字）。
...
的根目录，并按照grldr.dbr的第二扇区建立它的引导扇区。然后这个FAT12/16的卷就是GRUB可启动的。
...
02h	1	LBA指示码。CHS模式时，有效值为0x90；LBA模式时，有效值为0x0e 。
...
目录，并按照grldr.dbr的第一扇区建立它的引导扇区。然后这个FAT32的卷就是GRUB可启动的。
...
02h	1	LBA指示码。CHS模式时，有效值为0x90,而LBA模式时，有效值为0x0e。
...
目录，并按照grldr.dbr的第7至10扇区建立它的引导扇区。然后这个NTFS的卷就是GRUB可启动的。
...
02h	1	LBA指示码。CHS模式时,有效值为0x90；LAB模式时，有效值为0x0e。
...
目录，并按照grldr.dbr的第5至6扇区建立它的引导扇区。然后这个exFAT的卷就是GRUB可启动的。


偏移    长度    说明
======	======	==============================================================
00h	3	跳转代码。  
03h	5	分区类型标志，即“EXFAT”。
08h	3	202020
0Bh	53	全部为“00”。
...
03h	1	等待键被按下时的暂停秒数。0xff代表始终暂停（即无休止的）。


04h	2	热键代码。高字节是扫描码，低字节是ASCII码。默认值是0x3920，代
                表的是空格键。如果此键被按下，GRUB将在引导原先的主引导记录之前
		启动。见“ int16 键盘扫描码”。

06h	1	优先引导的驱动器号，0xff 代表没有。

07h	1	优先引导的分区号，0xff 代表整个驱动器。
...
选项：
	...
	--ext2			--floppy选项使用时，确认安装到EXT2/3/4系统。

	--exfat			--floppy选项使用时，确认安装到EXFAT系统。
...
注意 3：如果DEVICE_OR_FILE是软驱或软盘映像文件，而它又被格式化为EXT2/3/4，
...
步骤 1. 获取分区的引导扇区然后保存为一个文件 MYPART.TMP 。对于 NTFS 文件系统,你需要获取
        ...
        也是没问题的。
	更新：对于EXT2/3/4分区，需要获取起始的3个扇区，对于其他类型的文件系统，你只需要获取一个扇区。
...
步骤 2. 执行这些命令：
	...
	更新：对于FAT12/16/32，NTFS，EXT2/3/4,EXFAT等分区，只需要指定参数 --floppy。
		bootlace.com --floppy 文件名

步骤 3. 将 MYPART.TMP 写回你原来分区(hdx,y)的引导扇区。

注意： 现在只有一些文件系统（FAT12/16/32/NTFS/ext2/ext3/ext4/exfat）被支持。
