...
在 grldr 的偏移 0x1f9 处有一开关。
  按 F2 键无条件加载。

在 grldr 的偏移 0x1fb 处有一开关。
  00表示：不加载；
  01表示：当启动设备被识别为软盘时加载（默认）；
  02表示：始终加载。

在加载 usb 驱动之初，还有一次5秒提示：按 s 键降速加载；按空格键不加载。
等待秒数可由 grldr 的偏移 0x21b1 处设置。
速度设定可由 grldr 的偏移 0x21b2 处设置。（0=全速，1=全速/2，2=全速/4，3=全速/8）

设置：
	--grldr-usb=I 	I是加载开关	（此选项必须有）
	--usb-time=T 	T是等待秒数
	--usb-speed=S	S是速度
	--usb-key=K	K是无条件加载扫描码

例：bootlace --floppy --grldr-usb=2 --usb-time=3 grldr
