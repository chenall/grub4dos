...
说明:
	在一行中运行多个命令,相当于多行命令.顺序执行,前面的命令出错时也不会有错误警告.
	适用于像iftitle之类的场合.
...
更新：增加 --usb=I 参数，可以安装 usb2.0 驱动。其中 I 表示：
	01：不支持扩展读写时加载，其他不加载；
	02：不论是否支持扩展读写，均加载。

重要提醒：对于方法1，ext2 文件系统加载 usb 驱动时，回写位置要提前 0x0a 引导扇区！
...
作为一种补救措施，加载 usb 驱动后，可以支持扩展读，解决 chs 识别问题。
...
usb 驱动代码一般应放在 PBR 扇区之后。

PBR 引导程序首先加载 usb 驱动，然后调用它，驻留在常规内存顶部，通过拦截 int13 来读写 usb 设备。 

在 PBR 的偏移 0x1fb 处有一开关。
  01表示：不支持扩展读写时加载，其他不加载；
  02表示：不论是否支持扩展读写，均加载；
  00表示：不加载 usb 驱动。

在加载 usb 驱动之初，还有一次5秒提示：按 s 键降速加载；按其他键不加载；否则加载。
等待秒数可由 usb 驱动程序的偏移 0x0a 处设置。

注意：现在只有文件系统（FAT12/16/32/ext2/ext3/ext4/exfat）被支持。
注意：u盘预先需要格式化，取得 MBR 及 BPB 参数。
      
贴士：1. 有些 u 盘被 Windows 及 DOS 下的 usbaspi.sys 识别为 usb1.0 设备，但是
