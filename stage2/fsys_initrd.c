...
static int cpio_dir(const char* dirname)
{
	...
	{
		p_cpio_hdr = (struct cpio_header *)(grub_size_t)(initrdfs_base + cur_pos);
...
int initrdfs_mount (void)
{
	...
	else
	{
		for (i = 0; i < DRIVE_MAP_SIZE && !drive_map_slot_empty (disk_drive_map[i]); i++)
		{
			if (disk_drive_map[i].from_drive == (unsigned char)current_drive)
			{
				...
				{
					initrdfs_base = (grub_u64_t)disk_drive_map[i].start_sector << 9;
					initrdfs_size = (grub_u64_t)disk_drive_map[i].sector_count << 9;
				}
				break;
			}
		}
	...
	switch(*(grub_u32_t*)(grub_size_t)initrdfs_base)
	{
		case BAT_SIGN:
			initrdfs_type = 1;
			break;
...
int initrdfs_dir (char *dirname)
{
	...
	if (initrdfs_type == 1)
	{
		...
		grub_u32_t pos=0;
		grub_u32_t test = 0;
		cur_file.name = (grub_size_t)&filename;
		cur_file.isdir = 0;
