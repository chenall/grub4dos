...
only_tftp:
	...
	if (!pxe_opened)
	{
    printf_errinfo ("PXE cann't open %s.\n",name);
		return 0;
	}
  return 1;
}
...
//  efi_call_1 (grub_efi_system_table->boot_services->stall, 10000);  //延时10毫秒
  ...
  while (len)
  {
    ...
    response_message.body_length = len;         //响应消息.体长   设置为654800，他实际读5b4，
...
//  grub_efi_pxe_t *pxe = (is_ip6) ? net_devices->ip6_pxe : net_devices->ip4_pxe; //首选ip6,则选择ip6_pxe，否则选择ip4_pxe
  ...
  printf_debug ("PXE: %x\n", pxe);                         //PXE指针
  printf_debug ("PXE STARTED: %u\n", mode->started);       //PXE状态 0/1/2=停止/已启动/已初始化
  printf_debug ("PXE USING IPV6: %u\n", mode->using_ipv6); //PXE使用ipv4/ipv6=0/1
#if 0
  ...
  else
  {
    ...
    printf_debug ("ip4_manual_address=%x\n", manual_address);//e557780; e3d6860;
    printf_debug ("mode->station_ip.v4=%x,%x\n", mode->station_ip.v4,*(int*)mode->station_ip.v4);//f5851c4,272a8c0  站IP; f5851c4,272a8c0;
